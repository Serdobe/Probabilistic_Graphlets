...
per nodes. If this code is run, everything will be computed. Additionlly, this code
also prepares the pGDV and GDV to calculate the distances between orbits for the
enrichment analyses.
...
def Compute_Orbits_Spliting_Nodes(Network_Name, Total_Number_Nodes, file_Directory, save_directory, Run_path, threads = 1):
    ...
    os.chdir("/home/sergio/workspace_Eclipse/Lucky_GOGO_Extra/Results/Testeos_Varios/")
    ...
    for sentence in range(len(groups)):
        ...
        # New Option in G tries Scanner with the new implementation:
        '''
            - (-p)          Compute the probabilistic version of the counter (notice that -s has to be set to 3)
            - (-fp)         Save directory of the results
            - (-g0)         To count the orbit 0
            - (-paral)      The paralel version
            - (-From, -Tp)  the junk of nodes from the networks to analyze
            - (-s)          As the original, number of nodes in the graphlets (3, 4)
            - The rest of the arguments have the same rol as in the original code.
'''
        # Option to analyze the orbit 0 (-g0)
        ...
        # Option to analyze the graphlets of three nodes ()
...
# Function to compute Tijana distance: 

    
def Run_Tijana_Distance(Name_Network, save_directory, Names_Genes):
    
    '''

        For this function you need the distance cpp code (included in the folder).

    '''
    
    Tijana = "/home/sergio/workspace_Eclipse/Lucky_GOGO_Extra/Results/Python_Srcipts/Final_Results/Genetic_Interactions/Tijana_Distance/a.out"
    imput = [save_directory + "_Temporal_File" + str(Name_Network)]
    output = [save_directory + "Tijana_Distance_Final" + str(Name_Network)]
    
    cmd = [Tijana, imput[0], output[0]]
    ...
    D = np.genfromtxt(output[0])
    D = pd.DataFrame(D)
    D.columns = Names_Genes
    D.index = Names_Genes
    
    D.to_csv(save_directory + "_Result_Tijana_Final_" + Name_Network, 
...
Here this code will finish. New Distances could be applied to the main code and be added here
for the Enrichment analyses different pull of scripts were used.

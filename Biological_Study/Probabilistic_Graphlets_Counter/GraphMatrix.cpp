...
void GraphMatrix::make_probabilities_Matrix(Graph *g,vector<int> a, vector<int> b, int size, vector<float> c){


	//Get the number of nodes in the network:

	cout << "Loading the Probability Matrix" << endl;

	//Prueba

	std::vector<int> vector_nodes = a;

	std::copy(b.begin(), b.end(), std::back_inserter(vector_nodes));

	//vector_nodes.insert(vector_nodes.end(), b.begin(), b.end());
	std::sort(vector_nodes .begin(), vector_nodes .end());
	int nodes = std::unique(vector_nodes.begin(), vector_nodes.end()) - vector_nodes .begin();

	//Saving memory to fit the matrix (giving dimensions):

	Probability_Matrix.resize(nodes);

	for(int row(0); row< nodes; ++row){
		Probability_Matrix[row].resize(nodes);
	}

	//Fitting probability matrix: //first change <=

	//In the future take into account that maybe the name and number of nodes does not match and could be bad for this part!

	for(int position(0); position < size; ++position){
		Probability_Matrix[a[position]-1][b[position]-1] = c[position];
		Probability_Matrix[b[position]-1][a[position]-1] = c[position];
	}

	vector_nodes.clear();
}

float GraphMatrix::return_probability(int row, int column){


	float prob;

	prob = Probability_Matrix[row][column];

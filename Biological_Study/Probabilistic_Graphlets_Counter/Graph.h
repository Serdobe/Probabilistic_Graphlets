...
class Graph {
 public:
  ...
  //Information about the probability matrix in the object graph

  virtual vector<vector<float>> return_Probability_Matrix() = 0;
  virtual void make_probabilities_Matrix(Graph *g,vector<int> a, vector<int> b, int size, vector<float> c) = 0 ;
  virtual float return_probability(int row, int column) = 0;

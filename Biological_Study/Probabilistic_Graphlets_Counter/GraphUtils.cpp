...
void GraphUtils::readFileTxt(Graph *g, const char *s, bool dir, bool weight) {
  ...
  while (fscanf(f,"%d %d", &a, &b)==2) {
    if (weight) i=fscanf(f,"%f", &c);
    va.push_back(a);
    vb.push_back(b);
    vc.push_back(c); //For the vector of probabilities.
    if (a>max) max=a;
    if (b>max) max=b;
    size++;
  }
  ...
  ///Make the probability Matrix:
	  ...
	  cout << "Reading Probabilities" << endl;
	  g -> make_probabilities_Matrix(g,va, vb, size, vc);
	  cout << "Probability Matrix Created" << endl;
  ...
  va.clear();
  vb.clear();
  vc.clear(); //Free memory for the vector of probabilities.
}
